...
import {
  ...
  User, Pencil, Save, Eye, RefreshCw, History,
  Ban, Trash2, XCircle, RotateCcw, AlertTriangle, Check
} from "lucide-react";
...
const UserManagement = ({ plans, onDataRefresh }: UserManagementProps) => {
  ...
  const [confirmAction, setConfirmAction] = useState<"ban" | "unban" | "delete" | "reset_credits" | "cancel_sub" | null>(null);
  ...
  const handleBanUser = async (ban: boolean) => {
    ...
    if (error) { toast.error(error.message); return; }
    const res = data as any;
    if (!res?.success) { toast.error(res?.error === "cannot_ban_admin" ? "لا يمكن حظر مسؤول" : res?.error === "cannot_ban_self" ? "لا يمكن حظر نفسك" : res?.error || "فشلت العملية"); return; }
    toast.success(ban ? "تم حظر المستخدم" : "تم إلغاء الحظر");
    setConfirmAction(null);
    const updated = { ...selectedUser, is_banned: ban };
  ...
  const handleResetCredits = async () => {
    if (!selectedUser) return;
    setActionLoading(true);
    const { data, error } = await supabase.rpc("admin_reset_credits" as any, {
      p_target_user_id: selectedUser.id,
    });
    setActionLoading(false);
    if (error) { toast.error(error.message); return; }
    const res = data as any;
    if (!res?.success) { toast.error(res?.error || "فشلت العملية"); return; }
    toast.success(`تم تصفير الرصيد (كان ${res.old_balance})`);
    setConfirmAction(null);
    fetchUsers();
    onDataRefresh();
    openUserDetail(selectedUser);
  ...
  const handleCancelSub = async () => {
    if (!selectedUser) return;
    setActionLoading(true);
    const { data, error } = await supabase.rpc("admin_cancel_subscription" as any, {
      p_target_user_id: selectedUser.id,
    });
    setActionLoading(false);
    if (error) { toast.error(error.message); return; }
  ...
  const handleDeleteUser = async () => {
    if (!selectedUser) return;
    setActionLoading(true);
    // Delete user data from public tables (profile, credits, roles, transactions, generations, subscriptions)
    const userId = selectedUser.id;
    await Promise.all([
      supabase.from("credit_transactions").delete().eq("user_id", userId),
      supabase.from("credit_reservations").delete().eq("user_id", userId),
      supabase.from("generations").delete().eq("user_id", userId),
      supabase.from("user_subscriptions").delete().eq("user_id", userId),
      supabase.from("trial_requests").delete().eq("user_id", userId),
      supabase.from("phone_verifications").delete().eq("user_id", userId),
  ...
  const executeConfirmedAction = () => {
    switch (confirmAction) {
      case "ban": handleBanUser(true); break;
      case "unban": handleBanUser(false); break;
      case "delete": handleDeleteUser(); break;
      case "reset_credits": handleResetCredits(); break;
      case "cancel_sub": handleCancelSub(); break;
    }
  };

  const confirmMessages: Record<string, { title: string; desc: string; color: string }> = {
    ban: { title: "حظر المستخدم", desc: "سيتم حظر المستخدم وإلغاء اشتراكه النشط. لن يتمكن من استخدام المنصة.", color: "border-amber-500/50" },
    unban: { title: "إلغاء الحظر", desc: "سيتم إلغاء حظر المستخدم وسيتمكن من استخدام المنصة مجدداً.", color: "border-green-500/50" },
    delete: { title: "حذف بيانات المستخدم", desc: "سيتم حذف جميع بيانات المستخدم نهائياً: الملف الشخصي، الرصيد، الاشتراكات، والتوليدات. هذا الإجراء لا يمكن التراجع عنه!", color: "border-destructive/50" },
    reset_credits: { title: "تصفير الرصيد", desc: "سيتم تصفير رصيد الكريدت لهذا المستخدم إلى صفر.", color: "border-amber-500/50" },
    cancel_sub: { title: "إلغاء الاشتراك", desc: "سيتم إلغاء الاشتراك النشط لهذا المستخدم فوراً.", color: "border-amber-500/50" },
  };
    ...
    return (
      <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-4">
        ...
        <div className="flex items-center gap-3">
          ...
          {roleBadge(role)}
          {isBanned && (
            <span className="text-[9px] font-bold px-2 py-0.5 rounded-full bg-destructive/15 text-destructive">محظور</span>
          )}
        </div>
        ...
        {isBanned && (
          <div className="bg-destructive/10 border border-destructive/30 rounded-xl p-3 flex items-center gap-3">
            ...
            <div>
              <p className="text-xs font-bold text-destructive">هذا المستخدم محظور</p>
              <p className="text-[10px] text-destructive/70">لا يمكنه استخدام المنصة حالياً</p>
            </div>
            <Button size="sm" variant="outline" className="mr-auto text-[10px] h-7 gap-1 border-green-500/30 text-green-400 hover:bg-green-500/10"
        ...
        {/* Profile Card */}
        <div className={`bg-card rounded-2xl border p-5 ${isBanned ? "border-destructive/30 opacity-75" : "border-border/50"}`}>
          <div className="flex items-start gap-4">
            <div className={`w-14 h-14 rounded-full flex items-center justify-center text-xl font-bold shrink-0 ${isBanned ? "bg-destructive/15 text-destructive" : "bg-primary/15 text-primary"}`}>
        ...
        <div className="grid grid-cols-2 gap-2">
          <Button size="sm" className="gap-1.5 text-xs" onClick={() => { setGrantOpen(!grantOpen); setSubOpen(false); setConfirmAction(null); }}>
        ...
        <div className="border-t border-border/30 pt-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            {!isBanned && (
              <Button size="sm" variant="outline" className="gap-1.5 text-[10px] h-8 border-amber-500/30 text-amber-400 hover:bg-amber-500/10"
                onClick={() => { setConfirmAction("ban"); setGrantOpen(false); setSubOpen(false); }}>
            ...
            {sub && (
              <Button size="sm" variant="outline" className="gap-1.5 text-[10px] h-8 border-amber-500/30 text-amber-400 hover:bg-amber-500/10"
                onClick={() => { setConfirmAction("cancel_sub"); setGrantOpen(false); setSubOpen(false); }}>
            ...
            {credits > 0 && (
              <Button size="sm" variant="outline" className="gap-1.5 text-[10px] h-8 border-amber-500/30 text-amber-400 hover:bg-amber-500/10"
                onClick={() => { setConfirmAction("reset_credits"); setGrantOpen(false); setSubOpen(false); }}>
                <RotateCcw className="w-3 h-3" />
                تصفير الرصيد
              </Button>
            )}
            <Button size="sm" variant="outline" className="gap-1.5 text-[10px] h-8 border-destructive/30 text-destructive hover:bg-destructive/10"
              onClick={() => { setConfirmAction("delete"); setGrantOpen(false); setSubOpen(false); }}>
        ...
        <AnimatePresence>
          {confirmAction && (
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
              className={`bg-card rounded-xl border-2 p-4 space-y-3 ${confirmMessages[confirmAction]?.color || "border-border/50"}`}>
              <div className="flex items-center gap-2">
                <AlertTriangle className={`w-5 h-5 ${confirmAction === "delete" ? "text-destructive" : confirmAction === "unban" ? "text-green-400" : "text-amber-400"}`} />
                <p className="text-sm font-bold text-foreground">{confirmMessages[confirmAction]?.title}</p>
              </div>
              <p className="text-xs text-muted-foreground">{confirmMessages[confirmAction]?.desc}</p>
              <div className="flex gap-2">
                <Button size="sm" variant={confirmAction === "delete" ? "destructive" : "default"} className="text-xs gap-1"
                  onClick={executeConfirmedAction} disabled={actionLoading}>
                  <Check className="w-3 h-3" />
                  {actionLoading ? "جاري التنفيذ..." : "تأكيد"}
                </Button>
                <Button size="sm" variant="ghost" className="text-xs" onClick={() => setConfirmAction(null)} disabled={actionLoading}>
                  إلغاء
                </Button>
              </div>
            </motion.div>
        ...
        {filteredUsers.map((u, i) => {
          ...
          return (
            <motion.div
              ...
              className={`bg-card rounded-xl border p-3.5 cursor-pointer hover:border-primary/30 hover:bg-card/80 transition-all group ${
                banned ? "border-destructive/30 opacity-70" : "border-border/50"
              }`}
            >
              <div className="flex items-center gap-3">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold shrink-0 group-hover:bg-primary/25 transition-colors ${
                  banned ? "bg-destructive/15 text-destructive" : "bg-primary/15 text-primary"
                }`}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    ...
                    {roleBadge(role)}
                    {banned && <span className="text-[8px] font-bold px-1.5 py-0.5 rounded-full bg-destructive/15 text-destructive">محظور</span>}
                  </div>
